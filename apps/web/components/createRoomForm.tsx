  ...
  const submit = async (data :{
    ...
    password? : string 
      ...
      if(action === "create"){
        try{
          setLoading(true)
          const res = await axios.post(`${process.env.NEXT_PUBLIC_BETTER_AUTH_URL}/api/room/create` , {
            name : data.name ,
      ...
      if(action === "join"){
       const res = await axios.post(`${process.env.NEXT_PUBLIC_BETTER_AUTH_URL}/api/room/join` ,{
        name : data.name ,
        password : data.password 
       },{
        withCredentials :true
